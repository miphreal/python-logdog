...
class TestMsg(object):
    ...
    def test_serialize_json(self):
        ...
        m = Msg(message='сообщение', source='/tmp/x',
                meta={'tags': ['syslog']})
        m.serialize_json()
    ...
    def test_deserialize_json(self):
        ...
        assert_that(Msg.deserialize_json(
            '{"msg": "log entry","src":"/tmp/x","meta":null,"_v":1}'),
                    has_properties(
                        message=equal_to('log entry'),
                        source=equal_to('/tmp/x'),
                        meta=None, version=1))
        assert_that(Msg.deserialize_json(
            '{"msg":"сообщение","src":"/tmp/файл","meta":null,"_v":1}'),
                    has_properties(
                        message=equal_to('сообщение'),
                        source=equal_to('/tmp/файл'),
                        meta=None, version=1))
    ...
    def test_serialize_jsonb(self):
        m = Msg(message='log entry', source='/tmp/x',
                meta={'tags': ['syslog']})
        m.serialize_jsonb()
        m = Msg(message='сообщение', source='/tmp/файл',
                meta={'tags': ['syslog']})
        m.serialize_jsonb()

    def test_deserialize_jsonb(self):
        m0 = Msg(message='log entry', source='/tmp/x',
                 meta={'tags': ['syslog']})
